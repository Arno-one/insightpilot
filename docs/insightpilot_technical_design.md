...
风险扫描现在由 `backend/app/workers/risk_jobs.py` 驱动，属于 Worker 内顺序流，不是真实 `LangGraph`。
...
### 12.1 `LangGraph` 仍是预留状态

虽然依赖已存在，但：

- `risk_analysis_graph.py` 还是占位
- `business_report_graph.py` 还是占位
- 真实主链路还没从 Worker 切换到图执行
...
- 兼容现有前端 Trace 页面
- 兼容现有节点命名认知
- 把 `risk_jobs.py` 从“大流程脚本”升级为“图节点 + 编排入口 + 持久化工具”

## 14. 一句话结论

当前技术方案已经从“先画蓝图”进入“把核心主链路做扎实”的阶段。数据库基线、RAG、审批、任务、日报、Trace 都已经形成闭环，下一步最关键的技术演进就是让风险扫描正式切到 `LangGraph`。
