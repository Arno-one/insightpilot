...
| Embedding | DashScope |
| Agent 编排 | `langgraph` 已落地在风险扫描图和经营日报图 |
...
经营日报现在也已经迁移到真实 `LangGraph` 图。

当前职责拆分为：

- `backend/app/modules/agent/graphs/business_report_graph.py`
  负责状态对象、节点函数、图编排和工作流执行。
- `backend/app/workers/report_jobs.py`
  只保留 RQ Worker 入口，转调图工作流。
...
- 经营日报图已经完成落地
- `report_jobs.py` 已缩成 Worker 入口

### 12.2 图模块本身开始变重

当前大文件问题已经从 Worker 转移到图模块本身，尤其是：
...
后续如果继续扩展，会更适合再往下拆 `state`、`nodes`、`repository`。
...
- 完成任务时自动写入 `crm_follow_up_record`
- 自动回写 `crm_customer.last_follow_up_at / next_follow_up_at / last_sentiment`
- 自动同步关联风险快照状态
...
当前技术方案已经从“先画蓝图”进入“让关键链路真实图编排化”的阶段。数据库基线、RAG、审批、任务、日报、Trace 都已经形成闭环，风险扫描图和经营日报图都已落地，后续重点会转向执行闭环和图模块进一步拆分。
