...
- 已实现销售任务列表。
- 已支持任务开始执行、完成、取消。
- 已支持任务完成后自动回写一条 CRM 跟进记录。
- 已支持任务完成或取消后同步关联风险快照状态。
- 已实现经营日报生成 Worker 与报告列表。
- 已实现 Agent Run 列表与详情。
- 已实现 Agent Step 时间线。
- 已实现 RAG Trace 与命中详情串联。
- 已完成风险扫描 `LangGraph` 图落地，Worker 已改为转调图工作流。
- 已完成经营日报 `LangGraph` 图落地，Worker 已改为转调图工作流。
- 前端已完成以下页面接入：
  - 经营驾驶舱
  - 风险中心
  - AI 审批台
...
- 经营日报已经迁到 `backend/app/modules/agent/graphs/business_report_graph.py`
- `backend/app/workers/report_jobs.py` 已缩成 Worker 入口
...
当前任务模块已经不再只是“审批通过后创建”和“列表查看”，而是补齐了最小执行闭环：
...
- 在当前最小闭环基础上继续补批量能力、审计和风险重算联动

### 5.2 全链路回归验证

目标：

- 确认双图迁移后，审批、任务、日报、Agent Trace 和 RAG 评估继续兼容
...
当前版本的重心已经从“补骨架”切到“做真实 Agent 编排 + 补真实执行回流”。数据库基线、演示数据、前端工作台和可观测链路都已经具备，风险扫描图、经营日报图和最小任务闭环都已落地，接下来更适合继续补全链路验证和更深的执行联动。
